...
"""
    ...
    ask_to_llm = requests.post(os.getenv("HACKCLUB_AI_URL"), headers=headers, json=payload)
    result = json.loads(ask_to_llm.json()["choices"][0]["message"]["content"])

    # Find the time and extract that specific clip from video
    video_index, start_second, end_second = itemgetter("video_index", "start_second", "end_second")(result)
